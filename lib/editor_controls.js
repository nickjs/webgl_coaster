...
THREE.EditorControls = function ( cameras, domElement ) {
  ...
  this.zoom = function ( distance ) {
    ...
    if ( object instanceof THREE.PerspectiveCamera ) {
      ...
      var zoom = Math.min(Math.max(object.zoom - (distance.z / 100), 2), 100);
      var x = window.innerWidth / zoom;
      var y = window.innerHeight / zoom;
  ...
  function pickCamera( event ) {
    ...
    if (LW.renderer.useQuadView) {
      camIndex = 0;
      if (event.clientX > window.innerWidth / 2) camIndex += 1;
      if (event.clientY > window.innerHeight / 2) camIndex += 2;
    } else {
      camIndex = 3;
    }
    scope.camera = object = cameras[camIndex];
  };
