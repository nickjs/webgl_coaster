...
THREE.TransformControls = function ( camera, domElement ) {
  ...
  var finalMoveEvent = { type: "finalMove" };
  ...
  function onPointerUp( event ) {
    ...
    if (scope.didMove) {
      scope.dispatchEvent( finalMoveEvent );
      scope.didMove = false;
    }
  }
  ...
  function intersectObjects( pointer, objects ) {
    ...
    if (LW.renderer.useQuadView) {
      ...
      pointerVector.set( ( x ) * 4 - 1, - ( y ) * 4 + 1, 0.5 );
    } else {
      pointerVector.set(x * 2 - 1, -y * 2 + 1, 0.5);
    }
    ...
    if (camera instanceof THREE.PerspectiveCamera) {
      ...
      var theRay = projector.pickingRay(pointerVector, camera)
      var intersections = theRay.intersectObjects(objects, true)
    }
